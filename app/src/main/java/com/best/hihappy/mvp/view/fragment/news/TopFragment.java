...
import com.best.hihappy.db.NewsData;
...
public class TopFragment extends BaseFragment implements NewsView, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void LoadData() {
        if (isVisible && isFirst) {
            Log.e(TAG, "LoadData: isVisible"+isVisible );
            Log.e(TAG, "LoadData: isFirst"+isFirst);
    ...
    private void initData(NewsBean newsBean) {
        ...
        for (int i = 0; i < dataBeanList.size(); i++) {
            ...
            newsData.setId(i);
            newsData.setCategory(dataBeanList.get(i).getCategory());
            newsData.setDataBean(dataBeanList.get(i));
            newsData.save();
        }
